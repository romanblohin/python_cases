...
class StarSky:
    ...
    def _create_stars(self):
        ...
        for row_number in range(number_rows):
            row_shift = randint(-10,10)
            print(row_shift)
            for star_number in range(number_stars_x):
                self._create_star(star_number, row_number, row_shift)

    def _create_star(self, star_number, row_number, row_shift):
        """Create an star and place it in the row."""
        star = Star(self)
        star_width, star_height = star.rect.size
        star.x = star_width + 2 * star_width * star_number + row_shift * 10
        star.rect.x = star.x
